...
app.get("/historicoIPCA/calculo", (req, res) => {
  ...
  if (isNaN(valor) || isNaN(dataInicioMes) || isNaN(dataInicialAno) || isNaN(dataFinalMes) || isNaN(dataFinalAno)) {
    return res.status(400).send("Erro: os valores devem conter apenas números");
  }

  //ano inicial e final entre 2015 e 2023
  if (dataInicialAno < 2015 || dataInicialAno > 2023 || dataFinalAno < 2015 || dataFinalAno > 2023) {
    return res.status(400).send("Erro: os anos devem estar entre 2015 e 2023");
  }

  //mes inicial e mes final entre 1 e 12
  if (dataInicioMes < 1 || dataInicioMes > 12 || dataFinalMes < 1 || dataFinalMes > 12) {
    return res.status(400).send("Erro: os meses devem estar entre 1 e 12");
  }

  //ano final for 2023 , mes final deve ser entre 1 e 5
  if (dataFinalAno === 2023 && (dataFinalMes < 1 || dataFinalMes > 5))  {
    return res.status(400).send("Erro: se o ano final for 2023, o mês final deve estar entre 1 e 5");
  }
  ...
  if(dataInicial > dataFinal) {
    return res.status(400).send("Erro: a data inicial não pode ser maior que a data final");
  }

  //calculo do IPCA
  const resultadoIPCA = calculoDeIPCA(valor, dataInicioMes, dataFinalMes, dataInicialAno, dataFinalAno);

  res.json({valor: valor, valorFinal: resultadoIPCA.toFixed(2), periodo: `${dataInicioMes}/${dataInicialAno} a ${dataFinalMes}/${dataFinalAno}`})
